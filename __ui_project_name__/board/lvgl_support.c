...
#include "src/gpu/lv_gpu_nxp_vglite.h"
#include "vg_lite.h"
#endif
/*******************************************************************************
 * Definitions
...
#define DEMO_FB_SIZE (DEMO_BUFFER_WIDTH * DEMO_BUFFER_HEIGHT * DEMO_BUFFER_BYTE_PER_PIXEL)
 ...
 * Variables
 ******************************************************************************/
static uint32_t s_frameBuffer[2];
...
#endif
static volatile bool s_transfering;
...
lv_color_t buf1[392*392];
lv_color_t buf2[392*392];

void lv_port_pre_init(void)
{
}

static void DEMO_CleanDCache(struct _disp_drv_t * disp_drv) {
...
void lv_port_disp_init(void)
{
    ...
    s_frameBuffer[0] = DEMO_BUFFER0_ADDR;
    s_frameBuffer[1] = DEMO_BUFFER1_ADDR;
    ...
    disp_drv.clean_dcache_cb = DEMO_CleanDCache;
    ...
    if (vg_lite_init(64, 64) != VG_LITE_SUCCESS) {
        LV_LOG_ERROR("VGLite init error. STOP.");
        vg_lite_close();
        assert(0);
    }
#endif
